...
public class HTMLString {  
   public static void main( String[] args ) throws IOException{  
       ...
       PrintStream out = new PrintStream(
      	        new FileOutputStream("/Users/hung8585/Desktop/output.doc", true), true);
          Document doc2 = null;
          System.setOut(out);
    	  ...
    	  Elements Province = doc2.select("table").select("tbody.admin0").select("tr").select("td");
    	   for (Element table : doc2.select("table").select("tbody")) {
               for (Element row : table.select("tr")) {
                   ...
                   if(tds.get(1).text().equals("Rural District") || tds.get(1).text().equals("Urban District") || tds.get(1).text().equals("District-level City") ) {
